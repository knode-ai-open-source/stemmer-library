...
COPY --chown=dev:dev . /workspace/stemmer-library
RUN mkdir -p /workspace/build/stemmer-library && \
    cd /workspace/build/stemmer-library && \
    cmake /workspace/stemmer-library && \
